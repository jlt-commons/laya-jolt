 ...
 *
 * Size: LEV_THREADS, else the online processors; lev_set_threads changes
 * it (1 = everything on the caller). Per-thread scratch (attention's
 * score blocks) lives with the pool and grows on demand. */

typedef void (*lev_task_fn)(void *ctx, int64_t task, int thread);
...
static int default_threads(void) {
    const char *env = getenv("LEV_THREADS");
    if (env && *env) {
        int n = atoi(env);
        if (n > 0) return n;
    }
...
int lev_threads(void) {
    if (pool.n == 0) pool_start(default_threads());
    return pool.n;
}

void lev_set_threads(int n) {
    pool_stop();
    pool_start(n);
}
 ...
 * when all are done */
static void lev_run(lev_task_fn fn, void *ctx, int64_t ntasks) {
    if (pool.n == 0) pool_start(default_threads());
...
static void *lev_scratch(int thread, size_t bytes) {
...
void lev_gather_rows(const float *emb, const int64_t *ids, int64_t n, int64_t d,
                     float *out) {
...
void lev_layernorm(const float *x, const float *w, const float *b, int64_t n,
                   int64_t d, float eps, float *out) {
    struct layernorm_job j = {x, w, b, n, d, eps, out};
    lev_run(layernorm_rows, &j, LLA_ROW_TASKS(n, 8));
}
 ...
 * by tensors_test. lev_gelu keeps libm's erff: it only runs on the
...
void lev_gelu(const float *x, int64_t n, float *out) {
    for (int64_t i = 0; i < n; i++) {
        float v = x[i];
...
void lev_relu(const float *x, int64_t n, float *out) {
    for (int64_t i = 0; i < n; i++) out[i] = x[i] > 0.0f ? x[i] : 0.0f;
}

void lev_silu(const float *x, int64_t n, float *out) {
    for (int64_t i = 0; i < n; i++) {
        float v = x[i];
        out[i] = v / (1.0f + expf(-v));
    }
...
void lev_swiglu(const float *x, int64_t n, int64_t mid, float *out) {
    struct swiglu_job j = {x, n, mid, out};
    lev_run(swiglu_rows, &j, LLA_ROW_TASKS(n, 4));
}
...
void lev_rope_tables(double theta, int64_t d, int64_t len, float *cos_t,
                     float *sin_t) {
    int64_t half = d / 2;
...
void lev_rope_apply(float *qk, const float *cos_t, const float *sin_t,
...
void lev_split_heads(const float *x, int64_t L, int64_t H, int64_t hd,
                     float *dst) {
...
void lev_split_qkv(const float *qkv, int64_t L, int64_t H, int64_t hd,
...
void lev_merge_heads(const float *src, int64_t H, int64_t L, int64_t hd,
                     float *x) {
    for (int64_t h = 0; h < H; h++) {
        for (int64_t t = 0; t < L; t++)
...
void lev_allowed_mask(const uint8_t *att, int64_t b, int64_t L, int64_t window,
                      uint8_t *out) {
...
/* ---------------- attention softmax (the two gemms are cblas, see lev.tensors/attention) ---------------- */
...
void lev_masked_softmax(const float *S, const uint8_t *allowed, int64_t lda,
 ...
 * same Accelerate / OpenBLAS entry lev.tensors/mmul! calls. The integer
 ...
 * torch's math-path SDPA runs; the softmax is lev_masked_softmax on the
...
static void attention_task(void *ctx, int64_t task, int thread) {
    ...
    float *S = lev_scratch(thread, (size_t)(2 * rows * cols) * sizeof(float));
    float *P = S + rows * cols;
    ...
    lev_masked_softmax(S, j->allowed + i0 * L + k0, L, rows, cols, P);
 ...
 * token-major ctx [L x H*hd]. q/k/v head-major [H*L x hd] (lev_split_qkv's
...
void lev_attention(void *sgemm, const float *qh, const float *kh, const float *vh,
    ...
    lev_run(attention_task, &j, H * j.nblocks);
}

/* ---------------- misc ---------------- */

/* out = a + b*alpha rowwise: [n x d]. Used for residual adds and type_emb. */
void lev_add_scaled(const float *a, const float *b, int64_t n, int64_t d,
...
void lev_add_bias(float *x, const float *bias, int64_t n, int64_t k) {
    for (int64_t i = 0; i < n; i++) {
        float *xi = x + i * k;
...
void lev_add_qtype_bias(const float *x, const float *bias, const int64_t *q,
...
void lev_softmax(const float *x, int64_t n, int64_t k, float *out) {
    for (int64_t i = 0; i < n; i++) {
...
float lev_max_abs_diff(const float *a, const float *b, int64_t n) {
    float m = 0.0f;
    for (int64_t i = 0; i < n; i++) {
...
/* Used by lev.prepare (jolt prepare): model.safetensors is F16 (one F32
...
static uint32_t lev_h2f_bits(uint32_t h) {
...
int64_t lev_f16_file_to_f32(const char *in, int64_t offset, int64_t count,
                            const char *out) {
    FILE *fi = fopen(in, "rb");
    if (!fi) return -1;
...
int64_t lev_copy_file_range(const char *in, int64_t offset, int64_t nbytes,
                            const char *out) {
    FILE *fi = fopen(in, "rb");
    if (!fi) return -1;
    FILE *fo = fopen(out, "wb");
...
int64_t lev_read_file_range(const char *path, int64_t offset, int64_t n, uint8_t *dst) {
 ...
 * golden/prepare.edn. */
int64_t lev_file_crc32(const char *path, int64_t *size_out) {
